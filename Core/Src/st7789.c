...
void ST7789_Fill_Color(uint16_t color)
{
    ...
    uint16_t full_loops = (ST7789_WIDTH * ST7789_HEIGHT) / (ST7789_WIDTH * HOR_LEN);
    uint16_t remainder  = (ST7789_WIDTH * ST7789_HEIGHT) % (ST7789_WIDTH * HOR_LEN);

    for (uint16_t i = 0; i < full_loops; i++)
        ST7789_WriteData((uint8_t *)disp_buf, sizeof(disp_buf));  // <-- cast aqui

    if (remainder)
        ST7789_WriteData((uint8_t *)disp_buf, remainder * sizeof(uint16_t));  // <-- e aqui
...
void ST7789_ClearArea(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
    ...
    // Preenche buffer (disp_buf é uint8_t!)
    for (uint32_t i = 0; i < sizeof(disp_buf); i += 2)
    {
        disp_buf[i]     = color >> 8;   // MSB
        disp_buf[i + 1] = color & 0xFF; // LSB
    }
    ...
    while (sent < total_pixels)
    {
        ...
        if ((total_pixels - sent) < chunk_pixels)
            chunk_pixels = total_pixels - sent;

        ST7789_WriteData((uint8_t *)disp_buf, chunk_pixels * 2);
        sent += chunk_pixels;
    }
